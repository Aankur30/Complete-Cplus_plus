...
#include <iostream>
using namespace std;
// non type template parameter makes code harder to read and understand for othe developer

template <typename T, size_t maximum> // whenever you do non type template parameter it will be part of all the templates inside the code
class BoxContainer
{
...
void BoxContainer<T, maximum>::add(const T &item)
{
    ...
    if (m_size == maximum)
    {
        cout << "Maximum capacity reched. Not adding item" << std::endl;
        return;
    }
